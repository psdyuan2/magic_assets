...
| Path | When | Script |
| --- | --- | --- |
| FX | Scale, move, filter, mask, flash. No image2 | `python3 .cursor/skills/magic-assets/run.py fx` |
| Grid | Same role, same cell size, smooth/clay/flat icons | `python3 .cursor/skills/magic-assets/run.py grid` |
| Pixel | Same cell size, but pixel-art / hard texel edges | `python3 .cursor/skills/magic-assets/run.py pixel` |
| Pixel-anim | One character, 4 consecutive pixel frames → GIF/strip | `python3 .cursor/skills/magic-assets/run.py pixel-anim` |
| Mixed | One large hero plus smaller matching objects | `python3 .cursor/skills/magic-assets/run.py mixed` |
...
2. List only the missing motion or rasters. Write names, style, and destination paths.
3. Pick fx, grid, pixel, pixel-anim, or mixed. Do not combine raster paths in one request. FX may wrap a finished raster.
4. Run `python3 .cursor/skills/magic-assets/run.py <path>` from the **host project root**. Do not hand-write image2 curl. FX must not call image2.
5. Inspect the pack. FX: open `preview.html`, then copy `fx.css` / `fx.js`. Rasters: inspect `sheet.png` for the 2px `#00FFFF` boxes, then `assets/*.png` (pixel also writes `.svg` / `palette.json`; pixel-anim cuts the sheet into `cycle.gif` / `strip.png` / `cycle.css` without redrawing). Re-cut rasters with `--skip-generate --sheet` if only the splitter is wrong. Tests: `python3 .cursor/skills/magic-assets/run.py test`.
...
Always run from the host project root:

```bash
python3 .cursor/skills/magic-assets/run.py fx --effects pulse,bob,flash,pop
python3 .cursor/skills/magic-assets/run.py grid --count 8 --names home,search,user,settings,bell,chart,folder,plus
python3 .cursor/skills/magic-assets/run.py pixel --count 4 --colors 12
python3 .cursor/skills/magic-assets/run.py pixel-anim --action idle --sprite 32
python3 .cursor/skills/magic-assets/run.py mixed --hero "handheld console front, original design" --small joystick,gamepad,cartridge
python3 .cursor/skills/magic-assets/run.py grid --skip-generate --sheet output/icon-sheet-8-xxx/sheet.png --count 8
python3 .cursor/skills/magic-assets/run.py test
```
...
Copy this whole folder. Do not copy repo-root leftovers.
...
- Script FX (no image2): [fx.md](fx.md)
- Equal-scale prompts and counts: [grid.md](grid.md)
- Pixel snap + rect SVG: [pixel.md](pixel.md)
- Pixel 4-frame cycles: [pixel-anim.md](pixel-anim.md)
- Mixed-scale prompts and split rules: [mixed.md](mixed.md)
